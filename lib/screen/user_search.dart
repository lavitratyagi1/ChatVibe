...
class _SearchPageState extends State<UserSearchPage> {
              ...
              child: _searchResults.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        var recipientId = _searchResults[index]
                            .id; // Assuming this is the recipient's UID
                        var senderId = widget
                            .userId; // Assuming you have the sender's UID stored in widget.userId

                        return GestureDetector(
                          onTap: () async {
                            // Check if a chat document exists between sender and recipient
                            bool chatExists =
                                await _checkExistingChat(recipientId, senderId);

                            if (chatExists) {
                              // If a chat document exists, navigate to the existing chat page
                              String chatDocumentId =
                                  await _getExistingChatDocumentId(
                              ...
                              String chatDocumentId =
                                  await _createNewMessageDocument(
                                      recipientId, senderId);
                          ...
                          child: ListTile(
                            title: Text(user['displayName'] ??
                                'Unknown User'), // Assuming 'displayName' is the key for the user's name
  ...
  Future<String> _createNewMessageDocument(
  ...
  Future<String> _getExistingChatDocumentId(
      String recipientId, String senderId) async {
    QuerySnapshot snapshot1 = await FirebaseFirestore.instance
        .collection('Messages')
        .where('senderId', isEqualTo: senderId)
